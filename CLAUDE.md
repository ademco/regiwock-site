...
Regi picked **Broadcast**, then round 3 made it a **camera screen**: the whole site is **one screen with no scrolling** that feels like a DSLR's live view (fishtank.live energy), on desktop and phone. Live muted YouTube streams are the footage. Dark, REC = Book me, tally red + AF green. Roadmap 2.2 (round 2) matches Regi's reference, a **Canon EOS live view + Canon menu**: lightweight plain white Barlow Semi Condensed (400/500, no bold or filled chips; tappable text turns gold) floating on the footage, thirds grid, a round mic-mute button for sound, ISO-style boxed labels on the readouts, orange dial marks on the exposure-scale stream picker, gold-cursor menu with colored tabs, numbered pages and SET OK screens, and ▶ SD-card playback of all work (file numbers, MVI_/SND_/IMG_ names, 3×3 index). Round 1's pixel-font "rugged LCD" was replaced. **Every readout must mean something real** (creating since, monthly listeners, followers, the visitor's time/date, clip count) and be tappable where useful; no fake counters. Work/About/Socials open as camera-menu panels. The **drawing motif is dropped**. The notebook pages below are what's live until 2.2 (tokens) and Phase 4 rebuild them. Sample: `/styleguide`. Readout numbers live in `STATS` in `src/config.ts`; **followers** come from `src/lib/followers.ts`, which pulls each platform's count at build time on Cloudflare (no API keys; Instagram/TikTok/Kick fall back to Jina Reader, r.jina.ai) and sums them. `/followers.json` on any preview shows which source answered per platform. The cloud sandbox can't reach those sites, so locally the readout shows `--`.
